...
import { estimateUsageTokens, handleEventAction, handleInboundMessage, normalizeInboundText, parseWechatMessage, resolveInboundAgent, sanitizeInboundUserText } from "../src/inbound.js";
...
test("handleInboundMessage degrades quickly when knowledge query is slow", async (t) => {
  const account = accountFixture(`acc-slow-knowledge-${Date.now()}-${Math.random()}`);
  account.knowledge = {
    mode: "external",
    providers: [{
      type: "dify",
  ...
  const result = await handleInboundMessage(account, {
    openId: "openid-slow",
    text: "slow knowledge query",
  });
  const elapsed = Date.now() - start;
  assert.equal(result.agentId, "wemp-kf");
  assert.equal(result.knowledgeHits, 0);
  assert.ok(elapsed < 1000, `expected degraded return under 1000ms, got ${elapsed}ms`);
});
