...
function readTranscribeEndpointFromConfig(account: ResolvedWempAccount): string {
  const accountConfig = account.config as Record<string, unknown> | null | undefined;
  const voiceTranscribe = accountConfig?.["voiceTranscribe"];
  if (!voiceTranscribe || typeof voiceTranscribe !== "object") return "";
  const endpoint = String((voiceTranscribe as Record<string, unknown>)["endpoint"] || "").trim();
  return endpoint;
}
...
async function withTimeoutStatus<T>(promise: Promise<T>, timeoutMs: number): Promise<{ value: T | null; timedOut: boolean }> {
