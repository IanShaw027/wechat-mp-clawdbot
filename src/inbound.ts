...
const KNOWLEDGE_SYNC_TIMEOUT_MS = Math.max(50, Number(process.env.WEMP_KNOWLEDGE_SYNC_TIMEOUT_MS || 500));
...
export async function handleInboundMessage(account: ResolvedWempAccount, event: MinimalInboundEvent): Promise<{ agentId: string; text: string; paired: boolean; assistantEnabled: boolean; knowledgeHits: number; usageExceeded: boolean; usage: { messages: number; tokens: number; day: string } }> {
  const inboundText = sanitizeInboundUserText(event.text);
  const paired = event.paired ?? isPairingAllowed(account.dm.allowFrom, account.accountId, event.openId);
  ...
  const knowledge = await withTimeout(
    searchKnowledge(inboundText, account.knowledge.mode, account.knowledge.providers).catch(() => []),
