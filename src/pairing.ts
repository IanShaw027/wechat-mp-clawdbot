...
function notifyEndpoint(): string {
  return String(
    process.env.WEMP_PAIRING_NOTIFY_ENDPOINT
    || process.env.WEMP_PAIRING_WEBHOOK
    || process.env.WEMP_PAIRING_ENDPOINT
    || "",
  ).trim();
}
...
function notifyAuthToken(): string {
  return String(
    process.env.WEMP_PAIRING_NOTIFY_TOKEN
    || process.env.WEMP_PAIRING_API_KEY
    || "",
  ).trim();
}
...
export function buildWempApproveHint(code: string): string {
  return `openclaw pairing approve wemp ${code} --notify`;
}
...
export function isPairingAllowed(allowFrom: string[], accountId: string, openId: string): boolean {
  const subject = buildPairingSubject(accountId, openId);
  if (allowFrom.includes(subject) || allowFrom.includes(openId)) return true;
  return approvedBySubject.has(subject);
}
